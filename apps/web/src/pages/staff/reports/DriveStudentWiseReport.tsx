...
/** The flip side of Drive-wise Summary (DriveSummaryReport.tsx) — that one
 * aggregates the applied→selected funnel per drive; this flattens every
 * application to one row per student per drive instead, so you can trace
 * a single student across every drive they've applied to, or export the
 * whole department's application history in one file. Same data
 * (useAllApplications) as the drive-wise version, just unaggregated. */
export default function DriveStudentWiseReport() {
  ...
  // Only drives actually present among applications, not every drive ever
  // created — same "built from loaded data" convention as batchYearOptions.
  const driveOptions = useMemo(() => {
    if (!applications) return [];
    const ids = Array.from(new Set(applications.map((a) => a.driveId)));
    return ids
  ...
  }, [applications, studentsByUid, drives]);
  ...
  function handleDownload() {
    if (!rows) return;
    downloadCsv(
      "drive-student-wise-report.csv",
      ["Roll No", "Name", "Department", "Batch", "Company", "Role", "Applied Date", "Status", "Current Round", "Attendance", "Last Updated"],
      rows.map((r) => [
        r.student!.rollNo,
        r.student!.name,
  ...
  return (
    <div>
      ...
      <PageHeader
        title="Student-wise Drive Summary"
        subtitle="Every application, one row per student per drive."
        icon={ListChecks}
      ...
      {!loading && allRows!.length === 0 && <EmptyState icon={ListChecks} title="No applications yet" />}
